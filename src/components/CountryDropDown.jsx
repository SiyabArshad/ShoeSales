...
export default function CountryDropDown({ data, selectedfunc, val }) {
  const [open, setOpen] = useState(false);
  const animatedHeight = useRef(new Animated.Value(0)).current;
console.log("i am showing from dm",data)
        ...
        <ScrollView showsVerticalScrollIndicator>
          {
            data?.map((item, i) => (
              <TouchableOpacity
                onPress={() => {
                  selectedfunc(item?.nicename);
                  toggleDropdown();
                }}
                key={i}
              >
                <BodyText size="m" text={item?.nicename} style={{ paddingHorizontal: rp(1), marginVertical: rp(1) }} />
              </TouchableOpacity>
            ))}
        </ScrollView>
      </Animated.View>
